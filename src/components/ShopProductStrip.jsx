...
function ProductCard({ product }) {
  return (
    <a
      ...
      className="group flex min-w-0 flex-1 flex-col rounded-md bg-white p-1.5 ring-1 ring-gray-100 transition-colors hover:bg-gray-50 hover:ring-gray-200"
    >
      ...
      {product.source && product.title && (
        <p className="mt-0.5 truncate text-[10px] text-gray-400">{product.source}</p>
      )}
      {product.price && (
 ...
 */
export default function ShopProductStrip({ item }) {
  const [status, setStatus] = useState('idle') // idle | loading | ready | empty
  const [products, setProducts] = useState([])

  const query = useMemo(() => buildShopQuery(item), [
    item?.material,
    item?.category,
  ...
  const maxPrice = useMemo(() => getItemMaxPrice(item), [
    item?.budgetMax,
    item?.priceOptions,
  ])
